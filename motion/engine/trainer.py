...
class DefaultTrainer(BaseTrainer):
    ...
    @call_func_monitor
    @gpu_memory_clear
    def test(cls, cfg, pipeline, device, epoch, split="valid"):
        results = []
        ...
        return pipeline.compute_results(results, cfg, epoch)
    ...
    @call_func_monitor
    def do_eval(self, split="valid"):
        self.logger.info(f"Start to evaluate on {split} dataset...")
        return self.test(self.cfg, self.pipeline, self.device, self.epochs, split)
